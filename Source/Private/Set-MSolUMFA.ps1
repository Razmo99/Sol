        ...
        if ($PSCmdlet.ShouldProcess($UserPrincipalName, "StrongAuthenticationRequiremets = " + $StrongAuthenticationRequiremets)) {
            do {
                ...
                if (Get-MsolUser -UserPrincipalName $UserPrincipalName -ErrorAction SilentlyContinue) {
                    $Finished = $true
                    Write-Log -Level Debug -Message 'Found {0} In Msol' -Arguments $UserPrincipalName
                    Write-Log -Level Debug -Message 'Attempting to Set MFA Status to Enforced'
                    ...
                    try {
                        ...
                        Write-Log -Level Debug -Message 'Set MFA Command Executed'
                        return $true
                    }
                    catch {
                ...
                elseif ($TimeNow -ge $TimeEnd) {
                    $Finished = $true
                    Write-Log -Level Debug -Message 'Failed to find user in Msol'
